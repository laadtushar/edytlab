...
- **`.github/workflows/release-win.yml`** (added in M02) handles Authenticode signing via `signtool` (SHA256, DigiCert timestamp) post-bundle. Verifies via `signtool verify /pa /v`.
...
## Required GitHub secrets for `release-win.yml`
...
- [x] `release-win.yml` references the secrets and signtool path
...
The M02 `release-win.yml` workflow is the gate.
