...
async fn live_round_trip() {
    ...
    let plan_steps_override = std::sync::Arc::new(std::sync::Mutex::new(None));
    let mut agent = ai::Agent::new(cfg, dispatcher, store, engine, plan_notify, plan_steps_override, clipboard);
