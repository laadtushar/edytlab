...
///
/// Unix-only: `fsync_dir` is a no-op on non-Unix, and `child.kill()` on
/// Windows uses `TerminateProcess` which is not equivalent to SIGKILL
/// for testing the rename window. A separate Windows-equivalent test is
